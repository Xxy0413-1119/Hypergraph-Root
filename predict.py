...
def evaluate(model, val_features, val_graphs, val_labels,  model_name, dpath):
    model.eval()
    ...
    print(" ".ljust(17,
                    ' ') + "MCC    " + "FSC    " + "Precision  " + "Recall  " + "accuracy  " + "sensitivity  " + "specificity  " + "AUC   ")
    print(root[1].ljust(17, ' ') + str(round(table[6], 3)).ljust(7, ' ') + str(round(table[0], 3)).ljust(7, ' ') + str(
        round(table[1], 3)).ljust(11, ' ') + \
          str(round(table[2], 3)).ljust(8, ' ') + str(round(table[3], 3)).ljust(10, ' ') + str(
        round(table[4], 3)).ljust(13, ' ') \
          + str(round(table[5], 3)).ljust(13, ' ') + str(round(rocauc, 3)).ljust(6, ' '))

    return str(round(table[6], 3)), str(round(table[0], 3)), str(round(table[1], 3)), str(round(table[2], 3)), str(
        round(table[3], 3)), str(round(table[4], 3)), str(round(table[5], 3)), str(round(rocauc, 3))
...
def main(model_name, data_path):
    ...
    if torch.cuda.is_available():
        model.cuda()
    val_features, val_graphs, val_labels = load_data(data_path + testset, data_path + testlabel,
                                                                Dataset_Path + "pssm/", Dataset_Path + "graph/", Prot_Path +"prot/")
    mcc, fsc, prec, recall, acc, sensi, speci, auc = evaluate(model, val_features, val_graphs, val_labels, model_name,
                                                              data_path)
    f = open("predict_result.txt", "a")
    f.write(
        mcc + " \t" + fsc + " \t" + prec + " \t" + recall + " \t" + acc + " \t" + sensi + " \t" + speci + " \t" + auc + "\n")
    f.close()
